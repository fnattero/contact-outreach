...
FIXED_RULE_PARAGRAPH = (
    "Estas instrucciones aplican sólo al cuerpo de la respuesta automática al cliente. "
    "No cambian las reglas de seguridad, la acción elegida ni el formato JSON que tenés "
    "que devolver."
)

EDITABLE_REPLY_PROMPT_BODY = "\n\n".join(
    (
...
OLD_EDITABLE_REPLY_PROMPT = "\n\n".join((FIXED_RULE_PARAGRAPH, EDITABLE_REPLY_PROMPT_BODY))


def remove_fixed_rule_from_editable_prompt(apps, schema_editor):
    del schema_editor
    prompt_configuration = apps.get_model("configuration", "PromptConfiguration")
    prompt_configuration.objects.filter(automatic_reply_prompt=OLD_EDITABLE_REPLY_PROMPT).update(
        automatic_reply_prompt=EDITABLE_REPLY_PROMPT_BODY
    )


def restore_fixed_rule_to_editable_prompt(apps, schema_editor):
    del schema_editor
    prompt_configuration = apps.get_model("configuration", "PromptConfiguration")
    prompt_configuration.objects.filter(automatic_reply_prompt=EDITABLE_REPLY_PROMPT_BODY).update(
        automatic_reply_prompt=OLD_EDITABLE_REPLY_PROMPT
    )


class Migration(migrations.Migration):
    dependencies = [
        ("configuration", "0014_update_default_automatic_reply_prompt"),
    ]

    operations = [
        migrations.RunPython(
            remove_fixed_rule_from_editable_prompt,
            restore_fixed_rule_to_editable_prompt,
        ),
    ]

...
def prospect_queryset(params: QueryDict) -> QuerySet[Prospect]:
    ...
    queryset = Prospect.objects.select_related("campaign").annotate(
...
def outbound_queryset(params: QueryDict) -> QuerySet[OutboundMessage]:
    queryset = OutboundMessage.objects.select_related("campaign", "prospect", "prospect_email")
    query = params.get("q", "").strip()
    if query:
        queryset = queryset.filter(
...
def response_queryset(params: QueryDict, *, owner_id: int) -> QuerySet[InboundMessage]:
    queryset = InboundMessage.objects.filter(connection__owner_id=owner_id).select_related(
...
def owner_campaigns(owner_id: int) -> QuerySet[Campaign]:
    return Campaign.objects.filter(created_by_id=owner_id).only("id", "name")
